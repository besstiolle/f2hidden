...
class file_showService extends abstractService implements interfaceService {
	
	protected $serviceName = 'file_show';

	protected $jsonBlock = 'shows';

	protected $currentEntity;

	private $type = EnumForgeFile::Show; 
	...
	function getAll(){
		...
		if(!isset($this->params['onTransfert']) || $this->params['onTransfert'] == 0) {
			...
			$dir = $config['root_path'].'/uploads/projects/'.$this->params['sid'].'/show/';
			$url = $config['root_url'].'/uploads/projects/'.$this->params['sid'].'/show/';
			$pattern = '/\.(gif|jpe?g|png)$/i';
			$files = file_showService::getFilesInDir($dir, $pattern);
			$count = ''.count($files);
