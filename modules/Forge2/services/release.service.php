...
class releaseService extends abstractService implements interfaceService {
	
	protected $serviceName = 'release';
	...
	public function __construct($path, $params){
		...
		$this->currentEntity = new Release();
	}
	...
	function getAll(){
		...
		if(!empty($this->params['package_id']) ) {
			$example->addCriteria('package_id', OrmTypeCriteria::$EQ, array($this->params['package_id']));
		}
	...
	function create(){
		...
		if(!empty($entities)){
			...
			$this->response->addContent('warn', 'entity with same name found for the package');
			return;
		}
